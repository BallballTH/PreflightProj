...
const apiUrl = Cypress.env("BACKEND_URL") || "http://localhost:3770";
    ...
    cy.request({
      ...
      expect(
        msg.includes("no image provided") ||
          msg.includes("name, detail, userid and status")
      ).to.eq(true);
    });
  });
